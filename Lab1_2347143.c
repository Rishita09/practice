...
void read(struct account *acc) //passing the structure through pointer hence access will be using ->
{
    ...
    scanf("%d", &acc->accId);
		...
		//calculating interest amount and total balance
...
void display(struct account acc) //passing the structure itself hence access will be using .
{
    ...
    printf("| %-16s | %-6d | %-11s | %-13.2f | %-14.2f | %-9.2f | %-12.2f | %-14.2f |\n",
        acc.holderName, acc.accId, acc.transType, acc.prAmount, acc.transAmount, acc.intRate, acc.intAmount, acc.totalBalance);
...
// Function to perform operations on the matrix formed from prAmount, transAmount, intRate, and totalBalance
void matOperations(struct account *finances, int count) {
    ...
    // Calculate the average transaction amounts for each type
...
int main() {
    ...
    do {
        ...
        switch (choice) {
			...
			case 4:
			    // Option to display account information
			    if (r <= 0) {
			        printf("Please enter the number of rows first.\n");
			    } else {
			        for (i = 0; i < r; i++) {
